...
type Name = String
type PcData = (PM.Position, PM.AbsoluteDirection, Name, Int, Int, Int, Int, [IT.Item])
type Phirc = String
...
savePc (PcDB pcdb) phirc pc =
  ...
  (CH.getPosition pc, CH.getDirection pc, CH.getName pc,
   CH.getMhp pc, CH.getHp pc, CH.getMmp pc, CH.getMp pc, CH.getItemList pc)
...
makePcDB phimap = return $ PcDB [("guest1", (case PM.loadPosition phimap "0:0" of
                                           Nothing -> undefined
                                           Just pos -> pos,
                                        PM.North,
